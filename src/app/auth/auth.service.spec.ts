...
import { User } from "../dashboard/pages/users/components/models"
...
describe('AuthService', () => {
    ...
    it('Si el login es valido el observable authUser$ debe emitir un valor', (done) => {
        ...
        //3- vemos si recibimos al usuario autenticado despues del login
        service.authUser$.subscribe({
            next: (authUser) => {
                expect(authUser).toBeTruthy(); expect(authUser).toEqual(mockUser);
                done();
            }
        })
    })
